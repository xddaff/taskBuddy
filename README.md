...
| `/dashboard` | Participation meter, recommended issues filtered by category, claim, sync |
| `/class` | Instructor only: set the minimum percentage, review the class and the category breakdown |

## Development

```bash
npm test          # unit tests for the categorizer, participation math, and recommender
npm run build     # generates the Prisma client, then builds Next.js
npm run db:seed   # reset the demo class fixtures
```

The categorizer in `src/lib/ai/`, the recommender in `src/lib/recommender/`, and the participation
math in `src/lib/participation.ts` are pure functions with no database or network access, which is
